...
The chain Binomial model has a single parameter, which is the the transmission probability, which is defined as the probability that an infected household member infects a susceptible household member.

This package contains functions related to the Chain Binomial probability distribution, as well as functions for estimating the transmission probability parameter and regression modelling relating the transmission probability to predictive factors. To get started the package need to be loaded as usual:
...
$$
is the per-person risk for getting infected, and depends on the the transmission probability $\theta$ and the number of infected individuals in generation $g$. This is the binomial probability of getting at least one "success" in $I_g$ binomial trials with probability $\theta$. This model is also referred to as the Reed-Frost model.
...
Consider a household with 4 persons. A single household member becomes infected by a contagious disease outside of the household, and the other 3 household members are susceptible to the disease. Assuming a transmission probability of 0.23, we can compute the probability that 2 of the 3 susceptible household members becomes infected as follows: 
...
```{r ex4}
dchainbinom(x = 0:3, s0 = 3, i0 = 1, prob = 0.23, generations = 2)
```
...
With an estimate of the transmission probability in families where the primary case was the father, a natural question would be what the SAR is when other family members are the primary case. The `cbmod` function let us do a regression analysis similar to the `glm` function, with predictors for the transmission probability. The `cbmod` function does not implement the formula interface as the `glm` function does, but you can use the `model.matrix` function instead. 
